...
export function ComponentLibrary() {
  ...
  const [waveplateModal, setWaveplateModal] = useState(false);

  const handleClick = (type: OpticalElementType) => {
    if (type === 'waveplate') {
      setWaveplateModal(true);
    } else {
      addElement(type);
    }
  };

  const handleWaveplateConfirm = (wtype: WaveplateType, angle: number, phase: number) => {
    addElement('waveplate', { waveplateType: wtype, angle, phaseRetardation: phase });
    setWaveplateModal(false);
  };
  ...
  return (
    <div className="flex flex-col h-full">
        ...
        {OPTICAL_COMPONENTS.map((comp, i) => {
          ...
          return (
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              className="w-full text-left flex items-center gap-3 p-3 rounded-lab-lg border transition-all duration-150 relative"
              style={{
            ...
            >
              ...
              {/* Add button */}
              <div className="flex items-center gap-1 px-2 py-1 rounded text-2xs font-semibold flex-shrink-0"
                style={{
      ...
      {/* Waveplate config modal */}
      <AnimatePresence>
        {waveplateModal && (
          <WaveplateModal
            onConfirm={handleWaveplateConfirm}
            onCancel={() => setWaveplateModal(false)}
          />
        )}
      </AnimatePresence>
    </div>
